...
#if (LOG_TIME_EN == 1)
#define __LOG__(fmt, loglevel, ...) printf("%s%s %s%-5s %s%s:%s%d %s" fmt, KBLU, sys_dbg_get_time(), KYEL, loglevel, KGRN, __SHORT_FILE__, KRED, __LINE__, KNRM, ##__VA_ARGS__)
#elif (__LOG_EN__ == 1)
#define __LOG__(fmt, loglevel, ...) printf("%s%-5s %s%s:%s%d %s" fmt, KYEL, loglevel, KGRN, __SHORT_FILE__, KRED, __LINE__, KNRM, ##__VA_ARGS__)
#else
