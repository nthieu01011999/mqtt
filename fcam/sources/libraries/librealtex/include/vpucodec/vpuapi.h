...
/* rts add */
#define RTS_VPU_SEC_AXI_SUPPORT_MAX_WIDTH_HEVC	2592
#define RTS_VPU_SEC_AXI_SUPPORT_MAX_WIDTH_AVC	2496
#define RTS_VPU_SEC_AXI_SUPPORT_MAX_HEIGHT	1944

#define RTS_VPU_SEC_AXI_SUPPORT_MAX_WIDTH_3917		3072
#define RTS_VPU_SEC_AXI_SUPPORT_MAX_HEIGHT_3917		2048
/* rts end */
...
/* rts add */
#define RTS_REF_COUNT_DEFAULT		2
/* rts definition: struct RtsBufAddr*/
typedef struct RtsBufAddr {
	Uint32 vbWork;
	Uint32 vbMV;
	Uint32 vbFbcYTbl;
	Uint32 vbFbcCTbl;
...
/* h264 level definition */
typedef enum {
	H264_ENC_LEVEL_1 = 10,
	H264_ENC_LEVEL_1_b = 99,
	H264_ENC_LEVEL_1_1 = 11,
	H264_ENC_LEVEL_1_2 = 12,
	H264_ENC_LEVEL_1_3 = 13,
	H264_ENC_LEVEL_2 = 20,
	H264_ENC_LEVEL_2_1 = 21,
	H264_ENC_LEVEL_2_2 = 22,
	H264_ENC_LEVEL_3 = 30,
	H264_ENC_LEVEL_3_1 = 31,
	H264_ENC_LEVEL_3_2 = 32,
	H264_ENC_LEVEL_4 = 40,
	H264_ENC_LEVEL_4_1 = 41,
	H264_ENC_LEVEL_4_2 = 42,
	H264_ENC_LEVEL_5 = 50,
	H264_ENC_LEVEL_5_1 = 51
} H264Level;

/* h265 level definition */
typedef enum {
	H265_ENC_LEVEL_1 = 30,
	H265_ENC_LEVEL_2 = 60,
	H265_ENC_LEVEL_2_1 = 63,
	H265_ENC_LEVEL_3 = 90,
	H265_ENC_LEVEL_3_1 = 93,
	H265_ENC_LEVEL_4 = 120,
	H265_ENC_LEVEL_4_1 = 123,
	H265_ENC_LEVEL_5 = 150,
	H265_ENC_LEVEL_5_1 = 153, /* wave521 h265 only support level<=5.0 */
	H265_ENC_LEVEL_5_2 = 156,
	H265_ENC_LEVEL_6 = 180,
	H265_ENC_LEVEL_6_1 = 183,
	H265_ENC_LEVEL_6_2 = 186,
} H265Level;
...
typedef enum {
    ...
    DEC_PIC_CRA_AS_BLA  = 0x20  /**< It handles CRA picture as BLA picture not to use reference from the previously decoded pictures. */
} DEC_PIC_OPTION;
...
typedef enum {
    ...
    RETCODE_IRB_FRAME_DROP,             /**< This means that source frame drop in Sub Frame Sync environment(WAVE5 only) */
...
*/
    ...
    ENC_CONFIG_SUB_FRAME_SYNC, /**< ENC_PIC_SUB_FRAME_SYNC */
    ENC_SET_SUB_FRAME_SYNC, /**< P_MBC_SUB_FRAME_SYNC_IF */
    ENC_GET_SUB_FRAME_SYNC_REMAIN_CNT, /**< Get remain source count to encode.*/
    ENC_SET_FB_INPUT_RINGBUFFER,
/**
@verbatim
...
typedef struct {
    union {
        ...
        struct {
            ...
            int useEncVlcEnable; /**< This enables AXI secondary channel for VLC buffer. */
        } wave;
    } u;
} SecAxiUse;
...
*/
    ...
    BOOL            enReportRdoInfo; /**< It enables RDO info report. */
    PhysicalAddress reportDataAddr;  /**< It specifies the address of report data */
    BOOL            enReportVarInfo; /**< It enables Variance info report. */
...
#ifdef SUPPORT_CHANGE_QP_OFFSET
    int             gopQpOffset; /**< It sets QP offset. The final picQp = GopQp + gopQpOffset */
#endif
    Uint32 maxPicSize;
}EncWaveParam;
...
#ifdef SUPPORT_CHANGE_QP_OFFSET
    ENC_SET_CHANGE_PARAM_GOP_QP_OFFSET       = (1<<24),
#endif
    ENC_SET_CHANGE_MAX_PIC_SIZE              = (1<<25),
} Wave5ChangeParam;
/**
* @brief    This is an enumeration for declaring an reference ring buffer mode. .
*/
typedef enum {
    REF_RINGBUF_MODE_OFF       = 0,  /**< reference ring buffer mode off */
    REF_RINGBUF_MODE_SHORTTERM = 1,  /**< reference ring buffer mode with a single reference buffer */
    REF_RINGBUF_MODE_LONGTERM  = 2,  /**< reference ring buffer mode with dual reference buffers */
} RefRingBufMode;
/**
...
*/
    ...
    Uint32          vuiRbspDataSize;    /**< The bit size of the VUI rbsp data */
    PhysicalAddress vuiRbspDataAddr;    /**< The address of the VUI rbsp data */
    BOOL            enableInputRingBuffer; /**< It enables Input ringbuffer mode. */
    BOOL            ipuIndex;              /**< IPU index for Input ringbuffer mode */
    /**
    @verbatim
    The reference ring buffer mode enables to encode streams with a single compressed reference buffer or dual
    compressed reference buffers. See <<vpuapi_h_RefRingBufMode>>

    @* 0 : reference ring buffer mode off
    @* 1 : uses one reference ring buffer  (single reference buffer mode).
    @* 2 : uses two reference ring buffers (dual reference buffer mode).
    @endverbatim
    */
    RefRingBufMode  refRingBufMode;
    Int32           maxVerticalMV;  /**< The vertical motion vector range which is used for calculating reference ring buffer size. This value should be bigger than the absolute value of maximum negative vertical MV in the whole stream. */
    BOOL            enableSkipVLCBuffer; /**< It enables skip vlc buffer mode. */
    Uint8           cvbr_en;
    Uint16 	    max_br;
    Uint16 	    min_br;
    Uint8           I_dqp;
    ...
    Uint16          cvbr_var1;
    Uint16          cvbr_var2;
    Uint8           cvbr_mv_weight_scale;
    Uint8           cvbr_var_weight_scale;
} EncOpenParam;
...
/* rts add */
void VPU_PrintEncOpenParam(EncOpenParam *pop);
void VPU_PrintEncVpuStatus(EncHandle handle);
RetCode VPU_EncClose_Force(EncHandle handle);

/* rts add */
int VPU_SetIPUFrameMode(EncHandle handle, int mode);
int VPU_SetIPUFrameSync(EncHandle handle);
int VPU_SetIPUFrameSyncTHD(EncHandle handle, int thd);
int VPU_EnableIPUFrame(EncHandle handle);
int VPU_DisableIPUFrame(EncHandle handle);
int VPU_SetIPUHeight(EncHandle handle, uint32_t height);
int VPU_SetIPU_SYNC_METHOD(EncHandle handle, uint32_t val);
int VPU_StartIPUEncode(EncHandle handle);
int VPU_GetIPUDrop_cnt(EncHandle handle);
int VPU_ClearIPUDrop_cnt(EncHandle handle);
int VPU_CheckIPURingbuffer_status(EncHandle handle);
int VPU_ClearIPURingbuffer_status(EncHandle handle);
int VPU_GetIPUStart_line(EncHandle handle);

/* rts add */
int VPU_SetLowLatency_Mode(EncHandle Handle);
unsigned long long VPU_GetTimeStamp(EncHandle handle);
